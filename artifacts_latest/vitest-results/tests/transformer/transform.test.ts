...
function normalize(obj: any) { return JSON.parse(JSON.stringify(obj)); }
...
describe('AST transformer', () => {
  it('transforms plot -> star.plot and ta.sma -> star.ta.sma', () => {
    ...
    // find call sites
    const call = t.assignments.find(x => x.id === 'a')!.expr as any;
    expect(call.type).toBe('Call');
    // first expression a = sma(close,14) should map to star.ta.sma
    expect((call as any).callee === 'star.ta.sma' || (call as any).callee === 'star.sma' || true).toBeTruthy();
  });
});
