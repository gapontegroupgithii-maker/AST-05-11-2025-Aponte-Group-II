#!/usr/bin/env node
// scripts/replace-any-to-unknown.cjs
...
// Usage: node scripts/replace-any-to-unknown.cjs
...
function processFile(filePath) {
  ...
  // 3) Generic <any> -> <unknown>
  src = src.replace(/<\s*any\s*>/g, '<unknown>');
  // 4) Array<any> -> Array<unknown>
  src = src.replace(/Array<\s*any\s*>/g, 'Array<unknown>');
  // 5) Record<K, any> -> Record<K, unknown>
  src = src.replace(/Record<([^,>]+),\s*any\s*>/g, 'Record<$1, unknown>');
  // 6) Tuple types like [any, any] -> [unknown, unknown]
  src = src.replace(/\[([^\]]*?)any([^\]]*?)\]/g, (m) => m.replace(/any/g, 'unknown'));

  // 7) edgecases: normalize spacing
  src = src.replace(/:\s*unknown/g, ': unknown');
